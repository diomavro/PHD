...
To articulate $\beta$ a little more clearly, we can imagine $\pi^i_0$ if the project does not exists and $\pi^i_1$ if the project does in fact exist. So the excess profit from the project existing is given by, $\pi^i_1-\pi^i_0$. If we want to know what the excess profit is as a proportion of $\pi^i_0$ we need only divide, $\frac{\pi^i_1-\pi^i_0}{\pi^i_0}$ and this is the definition of $\beta$.
...
Similarly if we express $\pi^i > \pi^e + \alpha \pi^i $, we can note that $\frac{ \pi^i-\pi^e}{\pi^i} > \alpha $ is the condition for the firm to prefer to buy and shut down. Note then that $\pi^i-\pi^e$ is negative, then no $\alpha$ can satisfy the condition. So we can claim that if the incumbent buys a firm that is larger than itself, then the motivation is never to shut it down. 
