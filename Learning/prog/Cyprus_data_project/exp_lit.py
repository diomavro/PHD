...
def plot_gdp_growth(country_name):
    ...
    # Highlight and put on top points for the specified country
    country_data = filtered_data[filtered_data['Entity'] == country_name]
    ax.scatter(country_data['ln_rgdpo'], country_data['gdp_growth'], color='red', edgecolor='black', s=100, zorder=5,
               label=country_name)
    ...
    # Highlight and put on top points for the specified country in the second plot
    country_years = df_gdp[df_gdp['Entity'] == country_name]
    country_avg_log_growth = country_years.groupby('Year')['gdp_growth'].mean().reset_index()
    ax2.scatter(country_avg_log_growth['Year'], country_avg_log_growth['gdp_growth'], color='red', edgecolor='black', s=50,
                zorder=5, label=country_name)
    ax2.plot(country_avg_log_growth['Year'], country_avg_log_growth['gdp_growth'], color='red', linestyle=':', linewidth=1.5,
    ...
    # Add horizontal line for average log growth
    mean_gdp_growth = average_log_growth_per_year['gdp_growth'].mean()
    ax2.axhline(mean_gdp_growth, color='green', linestyle='--', linewidth=1, label='Mean Log Growth')
...
# Run the Streamlit app and specify the country name
country_name = st.text_input("Enter the country name:", "Cyprus")
if country_name:
    plot_gdp_growth(country_name)
