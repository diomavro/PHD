...
def portfolio_payoff(options: Tuple[List[Tuple[float, int]], List[Tuple[float, int]]],
                     spot_prices: List[float]) -> float:
...
def portfolio_cost(options: Tuple[List[Tuple[float, int]], List[Tuple[float, int]]], spot_prices: List[float],
                   markup: float) -> float:
...
def hedge(data: List[Tuple[float, float]], options: Tuple[List[Tuple[float, int]], List[Tuple[float, int]]],
          markup: float = 0.1, debug: bool = False) -> List[float]:
    ...
    incomes = [yield_rate * price + portfolio_value for (yield_rate, market_price), portfolio_value in
               zip(data, portfolio_values)]
    return incomes


def calculate_10p_ES_income_with_hedges(data: List[Tuple[float, float]], strikes: List[float],
                                        option_quantities: List[int]) -> float:
...
def calculate_10p_ES_income_with_hedges(data: List[Tuple[Tuple[float, float], float]],
    ...
    for (yield_, market_price), portfolio_value in data:
        hedged_portfolio_value = portfolio_value - option_portfolio_cost + option_portfolio_payoffs[
            market_prices.index(market_price)]
    ...
    hedged_portfolio_incomes = np.array(
        [yield_ * market_price + hedged_portfolio_value for (yield_, market_price), hedged_portfolio_value in
         zip(data, hedged_portfolio_values)])
...
def calculate_10p_ES_income_with_hedges(option_quantiles: List[float]) -> float:
    ...
    """
    # Hedge the portfolio
    hedge_results = np.array(
        hedge(global_data, (list(zip(strikes, tuple(option_quantiles))), ()), markup=0.1, DBG=False))
...
def portfolio_cost(options: Tuple[List[Tuple[float, int]], List[Tuple[float, int]]], prices: List[float],
                   markup: float) -> float:
    """
