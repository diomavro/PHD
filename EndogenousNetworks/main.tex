...
\begin{align*}
& (\text{IR}_i) ~~ \pi_i(i, \text{g})\geq 0~~\Leftrightarrow~~ p_i(i,\text{g})-r^{i-1}_i(\text{g})+r^{+}_i(\text{g})-F \geq 0\\
&  (\text{IC}_i) ~~ \pi_i(i, \text{g})\geq \max\{\pi_i(\tilde{k}_i,\tilde{\text{g}}),0\} ~~\Leftrightarrow~~ p_i(i,\text{g})-r^{i-1}_i(\text{g})+r^{+}_i(\text{g})-F \geq \max\{p_i(\tilde{k}_i,\tilde{\text{g}})-r^{h}_i(\tilde{\text{g}})-F,0\}
\end{align*}
...
\begin{align*}
(\text{IC}_i)~~& r^{i-1}_i(\text{g})= \max\{\Delta p_i+ r^-_i(\tilde{\text{g}})~,~ p_i(i,\text{g})-F\}+ r^+_i(\text{g})\\
\Leftrightarrow ~~& r^{i-1}_i(\text{g})= \max\{\Delta p_i+ r^-_i(\tilde{\text{g}})~,~ p_i(i,\text{g})-F\}+ [r^+_i(\text{g})-r^{i+1}_i(\text{g})] + r^{i+1}_i(\text{g})
\end{align*}
...
\begin{equation*}
r^{i-1}_i(\text{g})= \sum_{l=i}^{m} ~\max\{\Delta p_l + r^-_l(\tilde{g}) ~,~ p_l(l,\text{g})-F\}+[r^+_i(\text{g})-r^{i+1}_i(\text{g})],
\end{equation*}
where every term of the sum is positive (otherwise $l$ would be better off not entering the industry, for any $i\leq l\leq m$). It follows that $r^{i-1}_i(\text{g})$ is an increasing function of the index $i$ of a firm. Therefore the result. 
\end{proof}
