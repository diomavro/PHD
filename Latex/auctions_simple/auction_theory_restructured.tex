...
%   \begin{beamercolorbox}[ht=2ex,dp=1ex]{section in head/foot}
...
\begin{tikzpicture}[scale=0.9]
  % Traditional box
  \node[draw, thick, fill=blue!20, minimum width=2.5cm, minimum height=1.2cm, align=center] (trad) at (0,0) {Rules/\\Institutions};
  
  % Arrow
  \draw[->, ultra thick] (trad) -- (4,0);
  \node[above] at (2,0) {\small Traditional};
  ...
  \node[below] at (2,-2) {\small\color{red} Mechanism Design};
...
\begin{itemize}
  ...
  \item Mechanism $(x, p)$ where:
  \begin{itemize}
    \item $x: \Theta_1 \times \cdots \times \Theta_n \to X$ (allocation rule)
    \item $p: \Theta_1 \times \cdots \times \Theta_n \to \mathbb{R}^n$ (payment rule)
  \end{itemize}
\end{itemize}
...
\textbf{In Auctions:}
\begin{itemize}
  \item Second-price: DSIC and ex-post IR
  \item First-price: BIC and interim IR
\end{itemize}
\end{frame}
...
\textbf{Proof Sketch:}
\begin{enumerate}
  \item Start with arbitrary mechanism $M$ and equilibrium strategies $s^*(\theta)$
  \item Construct direct mechanism $M'$:
  \begin{itemize}
    \item Ask agents to report types $\hat{\theta}$
    \item Apply equilibrium strategies: $s^*(\hat{\theta})$
    \item Run original mechanism $M$ with these strategies
  \end{itemize}
  \item In $M'$, truthful reporting is optimal because:
  $$U_i(\theta_i, M') = U_i(s^*(\theta_i), M)$$
\end{enumerate}
...
\begin{itemize}
  \item Winner has highest valuation: $v_{(1)} = \max\{v_1, \ldots, v_n\}$
...
\begin{itemize}
  ...
  \item $n=3$: $\mathbb{E}[v_{(1)}] = \frac{3}{4}$, $\mathbb{E}[v_{(2)}] = \frac{1}{2}$
...
\begin{itemize}
  ...
  \item Values drawn i.i.d. from $F$
...
% Plot 1: Purple shape - Common knowledge
\begin{tikzpicture}[scale=0.4]
    \fill[yellow!30!white] (0,0) circle (3.5cm);
    \draw[very thick, black] (0,0) circle (3.5cm);
    
    % Large purple shape covering the entire top part of the circle - extends below red line
    ...
    % Label at bottom
    \node[align=center, font=\small\bfseries] at (-6, 0) {Common\\knowledge};
\end{tikzpicture}
\hspace{0.5cm}
% Plot 2: Green shape - Possible by blue but not red
\begin{tikzpicture}[scale=0.4]
    \fill[yellow!30!white] (0,0) circle (3.5cm);
    \draw[very thick, black] (0,0) circle (3.5cm);
    
    % Green shape in the right blue quadrant that intersects the red line
    ...
    % Label at bottom
    \node[align=center, font=\small\bfseries] at (7.5, 0) {Never knowable\\but ``possible'' by\\blue, not red};
\end{tikzpicture}

% Plot 3: Orange shape - Knowledge only blue can know
\begin{tikzpicture}[scale=0.4]
    \fill[yellow!30!white] (0,0) circle (3.5cm);
    \draw[very thick, black] (0,0) circle (3.5cm);
    ...
    \draw[very thick, orange!80!red, fill=orange!50] 
        ...
        -- (2.5, -2.5) -- (0, 0)
        to[bend right=15] (-2.2, 0.6)
        to[bend left=10] (-2.6, -0.3)
        to[bend right=8] (-2.5, -2.5);
    
    \draw[very thick, orange!80!red, fill=orange!50] 
        (0, 0) -- (2.5, -2.5) 
        to[bend left=8] (2.6, -0.3)
        to[bend right=10] (2.2, 0.6)
        to[bend left=15] (0, 0);
    ...
    % Label at bottom
    \node[align=center, font=\small\bfseries] at (-7, 0) {Knowledge only\\blue can know};
\end{tikzpicture}
\hspace{0.5cm}
% Plot 4: Cyan shape - Unknowable by both
\begin{tikzpicture}[scale=0.4]
    \fill[yellow!30!white] (0,0) circle (3.5cm);
    \draw[very thick, black] (0,0) circle (3.5cm);
    
    % New shape that intersects both red horizontal and blue diagonal line
    ...
    % Label at bottom
    \node[align=center, font=\small\bfseries] at (6, 0) {Unknowable\\by both};
\end{tikzpicture}

\caption{Knowledge partitions for two agents with different information structures. The blue dotted lines represent the sharper agent. The red dotted line represents the coarser agent. Colored shapes represent events.}
\end{figure}
...
% Plot 1: Mutual knowledge of degree 1 - smallest region
\begin{tikzpicture}[scale=0.35]
    \fill[yellow!30!white] (0,0) circle (3.5cm);
    \draw[very thick, black] (0,0) circle (3.5cm);
    
    % Small purple shape similar to common knowledge but scaled down
    \draw[very thick, violet, fill=violet!40] 
...
\end{tikzpicture}
\hspace{0.5cm}
% Plot 2: Mutual knowledge of degree 2 - larger region
\begin{tikzpicture}[scale=0.35]
    \fill[yellow!30!white] (0,0) circle (3.5cm);
    \draw[very thick, black] (0,0) circle (3.5cm);
    
    % Medium purple shape similar to common knowledge
    \draw[very thick, violet, fill=violet!40] 
    ...
    % Label at bottom
    \node[align=center, font=\small\bfseries] at (6, 0) {$M^2(E)$};
\end{tikzpicture}

% Plot 3: Mutual knowledge of degree k - even larger region
\begin{tikzpicture}[scale=0.35]
    \fill[yellow!30!white] (0,0) circle (3.5cm);
    \draw[very thick, black] (0,0) circle (3.5cm);
    
    % Large purple shape similar to common knowledge, almost at full size
    \draw[very thick, violet, fill=violet!40] 
...
\end{tikzpicture}
\hspace{0.5cm}
% Plot 4: Common knowledge - entire upper region respecting partitions
\begin{tikzpicture}[scale=0.35]
    \fill[yellow!30!white] (0,0) circle (3.5cm);
    \draw[very thick, black] (0,0) circle (3.5cm);
    
    % Large purple shape covering the entire common knowledge region
    % This is the region that respects both partitions (coarsest common refinement)
    \draw[very thick, violet, fill=violet!40] 
        (-3.5, 0) arc[start angle=180, end angle=0, radius=3.5cm]
        to[bend left=10] (2.8, -0.3)
        to[bend right=8] (2.2, -0.6)
    ...
    \node[align=center, font=\small\bfseries] at (8, 0) {$C(E) = \lim_{k \to \infty} M^k(E)$};
\end{tikzpicture}

\caption{Progression from mutual knowledge to common knowledge. Mutual knowledge of degree $k$, denoted $M^k(E)$, represents the event that ``everyone knows that everyone knows that $\ldots$ (k times) $\ldots$ that $E$ occurred.'' Common knowledge $C(E)$ is the limit as $k \to \infty$.}
\end{figure}
\end{frame}
...
\begin{frame}{Dutch Auction: Strategic Equivalence to First-Price}
...
\begin{itemize}
  ...
  \item Identical payoff structures
...
\begin{tikzpicture}[scale=0.7]
  ...
  % Dutch auction
  \node[draw, circle, fill=green!20] (d1) at (0,0) {B1};
  \node[draw, circle, fill=green!20] (d2) at (2,0) {B2};
  \node[draw, circle, fill=green!20] (d3) at (4,0) {B3};
...
In symmetric equilibrium ($x_j = x^*$ for all $j \neq i$):
$$U_i = \frac{x_i^r}{x_i^r + (n-1)(x^*)^r} \cdot V - x_i$$

\textbf{FOC:}
$$\frac{r x_i^{r-1} \cdot (n-1)(x^*)^r}{[x_i^r + (n-1)(x^*)^r]^2} \cdot V = 1$$

At $x_i = x^*$:
$$\frac{V \cdot r(n-1)(x^*)^{2r-1}}{[n \cdot x^*]^{2r}} = 1$$
...
$$\max_{e_i} \quad \Phi\left(\frac{e_i - e_j}{\sqrt{2}\sigma_\varepsilon}\right) u(w_1) + \left[1 - \Phi\left(\frac{e_i - e_j}{\sqrt{2}\sigma_\varepsilon}\right)\right] u(w_2) - e_i$$

\textbf{First-Order Condition:}
$$\phi\left(\frac{e_i - e_j}{\sqrt{2}\sigma_\varepsilon}\right) \cdot \frac{1}{\sqrt{2}\sigma_\varepsilon} \cdot [u(w_1) - u(w_2)] = 1$$

where $\phi(\cdot) = \frac{1}{\sqrt{2\pi}}e^{-x^2/2}$ is the standard normal PDF.

\textbf{Symmetric Equilibrium} ($e_i = e_j = e^*$):
$$\phi(0) \cdot \frac{1}{\sqrt{2}\sigma_\varepsilon} \cdot [u(w_1) - u(w_2)] = 1$$
$$\frac{1}{\sqrt{2\pi}} \cdot \frac{1}{\sqrt{2}\sigma_\varepsilon} \cdot [u(w_1) - u(w_2)] = 1$$
...
All four standard formats generate the same expected revenue!
\begin{center}
\begin{tikzpicture}[scale=0.68]
  % Four auction boxes
  \node[draw, thick, fill=blue!20, minimum width=1.8cm, minimum height=0.9cm, font=\small] (eng) at (-2,3) {English};
  \node[draw, thick, fill=green!20, minimum width=1.8cm, minimum height=0.9cm, font=\small] (vick) at (5.5,3) {Second-Price};
  \node[draw, thick, fill=orange!20, minimum width=1.8cm, minimum height=0.9cm, font=\small] (dutch) at (-2,1) {Dutch};
  \node[draw, thick, fill=purple!20, minimum width=1.8cm, minimum height=0.9cm, font=\small] (first) at (5.5,1) {First-Price};
  
  % Revenue
  \node[draw, circle, thick, fill=red!20, text width=1.2cm, align=center, font=\small] (rev) at (1.75,2) {Same\\Revenue};
...
\end{enumerate}
Then both mechanisms yield the same expected revenue.
\end{theorem}

\textbf{Key Implication:}
$$ER_{English} = ER_{Vickrey} = ER_{Dutch} = ER_{First} = \E[v_{(2)}]$$
...
\begin{itemize}
  ...
  \item Winner always pays "economic cost" of winning = second-highest value
\end{itemize}
\end{frame}

\begin{frame}{Why Revenue Equals $\E[v_{(2)}]$}
\textbf{English \& Vickrey:} Direct - winner literally pays $v_{(2)}$
$$ER_{SPA} = \E[v_{(2)}] = \int_{\underline{v}}^{\bar{v}} v \cdot g_{(2)}(v) \, dv$$

\textbf{First-Price \& Dutch:} Winner pays own bid $B(v_{(1)})$, but:
$$ER_{FPA} = \int_{\underline{v}}^{\bar{v}} B(v) \cdot g_{(1)}(v) \, dv$$

Using integration by parts with $B(v) = v - \frac{\int_{\underline{v}}^v F^{n-1}(t)dt}{F^{n-1}(v)}$:
\begin{align*}
ER_{FPA} &= \E[v_{(1)}] - \E\left[\frac{\int_{\underline{v}}^{v_{(1)}} F^{n-1}(t)dt}{F^{n-1}(v_{(1)})}\right] \\
&= \E[v_{(2)}]
\end{align*}

\textbf{Uniform $[0,1]$ Example:}
$$\E[v_{(2)}] = \frac{n-1}{n+1} \quad \text{for all formats!}$$
\end{frame}

\begin{frame}{Revenue Equivalence via Envelope Theorem}
\textbf{Setup:}
\begin{itemize}
  \item Allocation probability: $q_i(\hat{v}_i)$ (prob. of winning)
  \item Expected payment: $p_i(\hat{v}_i)$
  \item Utility: $U_i(v_i, \hat{v}_i) = v_i \cdot q_i(\hat{v}_i) - p_i(\hat{v}_i)$
\end{itemize}

\textbf{Equilibrium Value Function:}
\begin{align*}
V(v_i) &= \max_{\hat{v}_i} U_i(v_i, \hat{v}_i) = U_i(v_i, v_i) && \text{At equilibrium} \\
\frac{dV}{dv_i} &= \frac{\partial U_i}{\partial v_i}\Big|_{\hat{v}_i = v_i} = q_i(v_i) && \text{Envelope Theorem} \\
\rightarrow V(v_i) - V(\underline{v}) &= \int_{\underline{v}}^{v_i} q_i(t) \, dt &&
\end{align*}

With IR constraint $V(\underline{v}) = 0$:
$$\boxed{v_i \cdot q_i(v_i) - p_i(v_i) = \int_{\underline{v}}^{v_i} q_i(t) \, dt}$$

\textbf{Key Insight:} Expected payment $p_i(v_i)$ is uniquely determined by allocation rule $q_i(v_i)$!
\end{frame}

\begin{frame}{Proof of Revenue Equivalence}
From envelope formula:
\begin{align*}
p_i(v_i) &= v_i \cdot q_i(v_i) - \int_{\underline{v}}^{v_i} q_i(t) \, dt \\
\rightarrow \E[p_i(v_i)] &= \E[v_i \cdot q_i(v_i)] - \E\left[\int_{\underline{v}}^{v_i} q_i(t) \, dt\right]
\end{align*}

\textbf{Key Insight:} If two mechanisms have the same allocation rule $q_i(v_i)$, they have identical expected payments!

\textbf{Revenue Equivalence Conditions:}
\begin{enumerate}
  \item Same allocation rule: $q_i(v_i)$ identical across formats
  \item Same IR constraint: $V(\underline{v}) = 0$ for both
\end{enumerate}

\textbf{Application to Standard Auctions:}
\begin{itemize}
  \item English = Vickrey: Both give to highest $v$, truthful
  \item First-Price = Dutch: Both give to highest $v$, strategic
  \item All four give item to $v_{(1)}$ with probability 1
  \item Therefore: $\boxed{\text{All four have same revenue } = \E[v_{(2)}]}$
\end{itemize}
$\square$
\end{frame}
...
\textbf{Key Takeaway:} Revenue Equivalence holds under benchmark assumptions

\textbf{When It Breaks Down:}
\begin{itemize}
  \item Risk aversion: First-price $>$ Second-price
  \item Asymmetric bidders: Rankings depend on distributions
  \item Common values: English $>$ First-price $>$ Second-price
  \item Collusion: Sealed-bid $>$ English
\end{itemize}
\end{frame}
%===============================================
...
\begin{frame}{Optimal Auction Design: The Problem}
\textbf{Seller's Problem:}
\begin{itemize}
  \item Maximize expected revenue
  \item Subject to: IC (truthful reporting) and IR (participation)
  \item Design both allocation and payment rules
\end{itemize}

\textbf{Setup:}
\begin{itemize}
  \item $n$ bidders with values $v_i \sim F(v)$ on $[\underline{v}, \bar{v}]$
  \item Seller's value: $v_s$ (reserve value)
  \item Allocation: $q_i(v_1, \ldots, v_n) \in [0,1]$ (prob. $i$ wins)
  \item Payment: $p_i(v_1, \ldots, v_n)$
\end{itemize}

\textbf{Constraints:}
\begin{itemize}
  \item IC: Truthful reporting optimal
  \item IR: $\E[v_i q_i - p_i] \geq 0$
  \item Feasibility: $\sum_i q_i \leq 1$
\end{itemize}
\end{frame}

\begin{frame}{Virtual Valuations}
\begin{block}{Definition}
The \textbf{virtual valuation} of a bidder with value $v$, or effective value from seller's perspective is:
$$\psi(v) = v - \frac{1-F(v)}{f(v)}$$
Where $\frac{1-F(v)}{f(v)}$ is the rent extracted by high types
\end{block}

\textbf{Regularity Assumption:}
\begin{itemize}
  \item $\psi(v)$ is strictly increasing in $v$
  \item Holds if $f(v)$ is log-concave (most distributions)
  \item Ensures monotone allocation
\end{itemize}
...
\begin{theorem}[Myerson 1981: The revenue-maximizing auction]
\begin{enumerate}
  \item Allocates to bidder with highest positive virtual valuation:
  $$q_i(v) = \begin{cases} 1 & \text{if } \psi(v_i) = \max\{\psi(v_s), \psi(v_1), \ldots, \psi(v_n)\} \\ 0 & \text{otherwise} \end{cases}$$
  \item Sets reserve price where $\psi(r^*) = v_s$:
  $$r^* = v_s + \frac{1-F(r^*)}{f(r^*)}$$
  \item Charges payment from envelope formula
\end{enumerate}
\end{theorem}

\textbf{Implementation:}
\begin{itemize}
  \item Run second-price auction with reserve $r^*$
  \item Reject all bids below $r^*$
  \item Note: $r^* > v_s$ (set reserve \emph{above} own value!)
\end{itemize}
\end{frame}
...
\begin{frame}{Proof Sketch: Myerson Optimal Auction}
\begin{align*}
p_i(v_i) &= v_i q_i(v_i) - \int_{\underline{v}}^{v_i} q_i(t) \, dt && \text{From envelope} \\
\rightarrow \E[p_i] &= \E[v_i q_i(v_i)] - \E\left[\int_{\underline{v}}^{v_i} q_i(t) \, dt\right] && \\
&= \E\left[v_i q_i(v_i) - \frac{1-F(v_i)}{f(v_i)} q_i(v_i)\right] &&\\
&= \E[\psi(v_i) \cdot q_i(v_i)] && \\
\text{Total Revenue} &= \sum_i \E[\psi(v_i) \cdot q_i(v_i)] &&
\end{align*}

Maximize pointwise:
$$q_i(v) = \begin{cases} 1 & \text{if } i = \argmax_j \{\psi(v_s), \psi(v_1), \ldots, \psi(v_n)\} \\ 0 & \text{otherwise} \end{cases}$$

$\square$
\end{frame}

\begin{frame}{Optimal Reserve Price Derivation}
\textbf{From } $\psi(r^*) = v_s$:
$$r^* - \frac{1-F(r^*)}{f(r^*)} = v_s$$

\textbf{Interpretation:}
\begin{itemize}
  \item LHS: Virtual valuation at reserve
  \item RHS: Seller's outside option
  \item Set reserve where marginal bidder = seller value
\end{itemize}

\textbf{Uniform $[0,1]$ with $v_s = 0$:}
\begin{align*}
\psi(r^*) &= 2r^* - 1 = 0 \\
r^* &= \frac{1}{2}
\end{align*}
...
\textbf{Properties:}
\begin{itemize}
  \item $r^* > v_s$ (set reserve above own value!)
  \item Higher $r^*$ when $F$ has more mass at high values
  \item Trade-off: Higher price vs. no sale probability
  \item Independent of number of bidders for uniform distribution!
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item Pool of $N$ potential bidders
  \item Entry cost: $c > 0$ (paid to participate)
...
\begin{frame}{The Position Auction Problem: Google Ads Setting}
\begin{itemize}
  \item Multiple ad slots on search results page
  \item Slots differ in visibility (click-through rate)
  \item Advertisers compete for slots
  \item Pay per click, not per impression
\end{itemize}

\begin{center}
\begin{tikzpicture}[scale=0.80]
  % Ad slots
  \node[draw, thick, fill=yellow!30, minimum width=5.5cm, minimum height=1cm, align=left] at (0,2.5) {
    \textbf{Ad 1} - Top position (CTR = 0.10)
  };
  \node[draw, thick, fill=yellow!20, minimum width=5.5cm, minimum height=1cm, align=left] at (0,1) {
    \textbf{Ad 2} - Second position (CTR = 0.07)
  };
  \node[draw, thick, fill=yellow!10, minimum width=5.5cm, minimum height=1cm, align=left] at (0,-0.5) {
    \textbf{Ad 3} - Third position (CTR = 0.04)
  };
  
  % Organic results
  \node[draw, thick, fill=white, minimum width=5.5cm, minimum height=1cm] at (0,-2.3) {
...
\textbf{Interpretation:}
\begin{itemize}
  \item Happy with current position given prices
  \item No incentive to deviate to adjacent slot
  \item May still want to jump multiple positions
\end{itemize}

\textbf{Key Result:} Many equilibria exist in GSP!
\end{frame}

\begin{frame}{VCG for Position Auctions}
\textbf{VCG Applied to Positions:}

\textbf{Allocation:} Same as GSP - rank by value

\textbf{Payment:} Bidder $i$ in position $j$ pays:
$$p_i^{VCG} = \sum_{k=j+1}^{n} (v_k - v_{k+1})(\alpha_{k-1} - \alpha_k)$$

\textbf{Intuition:} Pay for displacing others downward
...
\begin{itemize}
  \item Simpler to explain
  \item Higher revenue in practice
...
\begin{itemize}
  \item Expected click-through rate (CTR)
  \item Ad relevance to query
  \item Landing page quality
  \item Historical account performance
\end{itemize}
...
\begin{itemize}
  ...
  \item Quality scores may be manipulated
...
\begin{itemize}
  \item Quality scores improve outcomes
  \item Price discovery through repeated auctions
  \item Learning and dynamics matter
  \item Simplicity valued over theoretical optimality
\end{itemize}
\end{frame}

\begin{frame}{Position Auctions: Mathematical Details}
\textbf{Expected Revenue per Bidder:}

In position $j$, bidder $i$ with value $v_i$ gets:
$$\text{Surplus}_i = \alpha_j \cdot (v_i - p_j)$$

where $\alpha_j$ is CTR and $p_j$ is price per click.

\textbf{Equilibrium Condition (GSP):}

Bidder $i$ in position $j$ with payment $p_j = b_{j+1}$ is happy if:
$$\alpha_j(v_i - b_{j+1}) \geq \alpha_{j-1}(v_i - b_j)$$
$$\alpha_j(v_i - b_{j+1}) \geq \alpha_{j+1}(v_i - b_{j+2})$$

\textbf{Rearranging:}
$$b_{j+1} \leq v_i - \frac{\alpha_{j-1}}{\alpha_j}(v_i - b_j)$$
$$b_{j+1} \geq v_i - \frac{\alpha_{j+1}}{\alpha_j}(v_i - b_{j+2})$$

Multiple bid vectors satisfy these constraints!
\end{frame}
...
\begin{itemize}
  \item Facebook/Instagram sponsored posts
  \item LinkedIn promoted content
  \item Position = visibility in feed
\end{itemize}

\textbf{3. E-commerce:}
\begin{itemize}
  \item Amazon sponsored products
  \item eBay promoted listings
...
\begin{itemize}
  \item YouTube ads
  \item Pre-roll, mid-roll positions
...
\begin{frame}{Complementarity vs. Substitutability}
\textbf{Complementarities:} Value of bundle exceeds sum of parts
$$v(S \cup T) > v(S) + v(T)$$
...
\textbf{Substitutes:} Value of bundle less than sum of parts
$$v(S \cup T) < v(S) + v(T)$$
...
\textbf{Challenge:} With $m$ items, $2^m$ possible bundles!
\begin{itemize}
  \item 10 items = 1,024 bundles
  \item 20 items = 1,048,576 bundles
  \item Cannot simply ask for all values
\end{itemize}
\end{frame}

\begin{frame}{Vickrey-Clarke-Groves: Efficient Truthful Bidding}
\begin{block}{Definition}
The \textbf{VCG mechanism} is:
\begin{enumerate}
  \item Bidders report valuations $v_i(S)$ for each bundle $S$
  \item Allocate to maximize total value:
  $$S^* = \argmax \sum_{i} v_i(S_i) \quad \text{s.t. } S_i \cap S_j = \emptyset$$
  \item Each winner $i$ pays:
  $$p_i = \underbrace{\max_{S_{-i}} \sum_{j \neq i} v_j(S_j)}_{\text{Without } i} - \underbrace{\sum_{j \neq i} v_j(S_j^*)}_{\text{With } i}$$
\end{enumerate}
\end{block}
...
\begin{itemize}
  ...
  \item \textbf{Bidder 1 pays:} $10 - 0 = 10$
  \item \textbf{Bidder 2 pays:} 0 (didn't win)
\end{itemize}
...
Bidder $i$'s utility if reports $\hat{v}_i$ when true value is $v_i$:
$$U_i = v_i(S_i^*) - p_i$$

where $S_i^*$ is what $i$ gets, and
$$p_i = \max_{S_{-i}} \sum_{j \neq i} v_j(S_j) - \sum_{j \neq i} v_j(S_j^*)$$

Substituting:
$$U_i = v_i(S_i^*) - \max_{S_{-i}} \sum_{j \neq i} v_j(S_j) + \sum_{j \neq i} v_j(S_j^*)$$

$$= \underbrace{\left[v_i(S_i^*) + \sum_{j \neq i} v_j(S_j^*)\right]}_{\text{Total value with } i} - \underbrace{\max_{S_{-i}} \sum_{j \neq i} v_j(S_j)}_{\text{Constant (doesn't depend on } \hat{v}_i)}$$

\textbf{Key:} Maximizing $U_i$ is same as maximizing total value!

Truth-telling achieves this maximum. $\square$
\end{frame}
...
\begin{itemize}
  ...
  \item Dominant strategy incentive compatible
...
\begin{frame}{SAA: Advantages and Disadvantages}
\textbf{Advantages:}
\begin{itemize}
  \item \textbf{Price discovery:} Bidders learn values through process
  \item \textbf{Computationally simple:} Bid on individual items
  \item \textbf{Flexible:} Can assemble packages as information revealed
  \item \textbf{Proven success:} Raised \$100+ billion for US government
\end{itemize}

\textbf{Disadvantages:}
\begin{itemize}
  \item \textbf{Exposure problem:} Risk of winning some but not all
  \begin{itemize}
    \item Bid on A and B, win only A (worth less alone)
  \end{itemize}
  \item \textbf{Not truthful:} Strategic bidding required
  \item \textbf{May be inefficient:} Due to exposure problem
  \item \textbf{Threshold problem:} Hard to compete with large packages
\end{itemize}

\textbf{Example (Exposure):}
\begin{itemize}
  \item Want $\{A, B\}$ valued at 15
  \item Items worth 2 each individually
  \item If win A but lose B, stuck with value 2
  \item May bid conservatively to avoid exposure
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item Auctioneer announces prices for each item
  \item Bidders indicate desired quantities at current prices
  \item Prices increase for over-demanded items
  \item Continue until supply = demand
\end{itemize}

\textbf{Phase 2: Supplementary Bids}
\begin{itemize}
  \item Bidders submit additional package bids
  \item Can bid on packages not available in clock phase
  \item Subject to activity constraints
\end{itemize}

\textbf{Phase 3: Winner Determination}
\begin{itemize}
  \item Find allocation maximizing total value
  \item Use all bids from both phases
  \item Solve optimization problem
\end{itemize}

\textbf{Phase 4: Pricing (Core Selecting)}
\begin{itemize}
  \item Set prices in the "core" (coalition-proof)
  \item Minimize revenue subject to core constraints
\end{itemize}
\end{frame}
...
\textbf{Interpretation:}
\begin{itemize}
  \item Winners cannot profitably block allocation
  \item Losers cannot outbid winners collectively, stability
\end{itemize}

\textbf{Core-Selecting Auctions:}
\begin{itemize}
  \item Choose prices in core that maximize revenue
  \item More revenue than VCG (typically)
  \item Still efficient allocation
  \item Used in modern spectrum auctions
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item Who should win?
  \item How to split gains?
  \item How to enforce agreement?
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item Small number of bidders (easier coordination)
  \item Frequent interactions (repeated game)
  \item Observable bids (detect deviations)
  \item Symmetric bidders (easier to agree)
\end{itemize}
\end{frame}
...
\begin{enumerate}
  ...
  \begin{itemize}
    \item No information revelation
    \item Hard to detect deviations
    \item Members uncertain about others' values
  \end{itemize}
  
  \item \textbf{Optimal response:}
  \begin{itemize}
    \item Use sealed-bid formats
    \item Set high reserve prices
    \item Encourage entry of new bidders
    \item Randomize auction procedures
  \end{itemize}
\end{enumerate}
...
\begin{itemize}
  ...
  \item Highly customized products
...
\begin{itemize}
  \item Use sealed-bid formats
  \item Set meaningful reserves
  \item Limit information disclosure
  \item Random participation rules
  \item Encourage new entrants
  \item Prosecute violations
\end{itemize}
...
\textbf{Applications:}
\begin{itemize}
  \item Stock exchanges (NYSE, NASDAQ)
  \item Commodity markets
  \item Electricity markets
  \item Carbon emission trading
  \item Decentralized prediction markets
\end{itemize}
...
\begin{itemize}
  \item No $k \in [0,1]$ achieves ex-post efficiency
  \item $k = \frac{1}{2}$ maximizes ex-ante gains from trade
...
\textbf{Implication:} Some efficiency loss is unavoidable in bilateral trade
\end{frame}
...
\begin{itemize}
  ...
  \item Use sealed bids when collusion risk high
...
\begin{itemize}
  \item Review current procurement processes
  \item Identify competitive bidding opportunities
\end{itemize}

\textbf{Short-term (This Month):}
\begin{itemize}
  \item Pilot reverse auction for non-critical spend
  \item Benchmark against industry practices
  \item Train team on fundamentals
\end{itemize}

\textbf{Long-term (This Quarter):}
\begin{itemize}
  \item Assess marketplace opportunities
