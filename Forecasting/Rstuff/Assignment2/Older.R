...
# and under adjusting in years years where sales are above average, as such the multipplicative adjustment is more appropriate

adtspassengers <- data2$AirPassengers / mdec$seasonal

#Create an seasonally adjusted and non adjusted dataset without missing values
...
train <- window(series1, start =1, end = 6 )
test <- window(series1, start =7, end = 11 )
...
tspower <- ts(data2$power , frequency = 24)
tsspeed <- ts(data2$speed , frequency = 24)

plot(tspower , type = "l", ylab = "power", xlab = "Hours" )

plot(x =data2$power, y = data2$speed, xlab = 'Power', ylab = 'Speed' )

cor(data2$power,data2$speed)

#Linear regression is just conditional correlation

regression <- lm(power ~  speed, data = data2)

summary(regression)

#This is our estimate for the next day, 
# 1)we estimate the intercept as a baseline and 
# 2) Multiply our expected speed by the coefficient
# 3a) Our expected speed can just be the average speed
# 3b) expected speed can just be the average speed at that time of day

mean(data2$speed)

#So our expected energy in the next day is the coefficient times the number of hours times the average hourly speed

17.68192*coefficient*24 + intercept

#Neural network
#Rescale the data

#Make dataset without the firs column the data
refined_train <- data2[-1]
refined_test <- data2[-1]

#We will use the first 2000 to train the the remaining to test
trainset <- head(refined, 2000)
testset <- tail(refined, 2544-2000 )
...
library(neuralnet)
set.seed(512)
mlp1 <- neuralnet(power ~ speed, refined=trainset_s,
